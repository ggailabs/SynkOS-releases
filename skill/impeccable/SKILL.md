...
Before any design work or file edits, you must register your identity.
Call `pane_set_identity` using the `SYNKO_PANE_ID` environment variable at the start of your session.
- **Skill**: `impeccable`
...
|---|---|---|
| Identity | If in SynkOS, call `pane_set_identity` using `SYNKO_PANE_ID`. | Register identity before continuing. |
